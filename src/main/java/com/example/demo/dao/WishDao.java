...
public interface WishDao extends JpaRepository<Wishes, Integer>, WishRepository {
	...
	@Query(value = "select * from wishes where LOWER(title) = LOWER(?1) and is_deleted = false and is_finished = false", nativeQuery = true)
	public List<Wishes> findActiveWishesByTitle(String title);
